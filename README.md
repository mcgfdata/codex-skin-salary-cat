...
| 月薪喵（默认） | 月薪喵 · 今日营业（横幅） |
|---|---|
| <img src="./presets/preset-yuexinmiao/background.jpg" alt="月薪喵默认样式预览"> | <img src="./presets/preset-yuexinmiao-payday/background.jpg" alt="月薪喵今日营业横幅样式预览"> |
...
设置完成后，两套样式都会出现在 Dream Skin 的“已保存的主题”中。默认使用“月薪喵”，用户可随时切换到“月薪喵 · 今日营业”，切换不需要重新下载素材或重复设置。
...
- 目标平台：macOS、Windows
- 样式 ID：`preset-yuexinmiao`（默认）、`preset-yuexinmiao-payday`（横幅）
...
它会把默认“月薪喵”设为活动主题，下次从 Codex Dream Skin 启动时直接生效。首次安装仍按上面的安全流程退出 Codex 后重跑 `Setup.command`。
...
- `月薪喵`：`2560 × 1440`，继续使用聚焦裁切，舍弃约 26% 横向画幅，主要来自左侧留白，优先放大右侧主体。
- `月薪喵 · 今日营业`：`2240 × 1600`（`1.40:1`），完整保留源图宽度，等比缩放后在取自原图的浅色画布底部对齐。宽高比低于模板的 `1.45` 宽图扩展阈值，因此首页稳定使用参考图中的横幅卡片，任务页由 `taskMode: banner` 使用顶部横幅。
...
- `presets/preset-yuexinmiao/`：默认布局，只含 `background.jpg` 和 `theme.json`
- `presets/preset-yuexinmiao-payday/`：今日营业横幅布局，只含 `background.jpg` 和 `theme.json`
