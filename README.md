...
只使用一张月薪喵素材制作的 [Codex-Dream-Skin](https://github.com/Fei-Away/Codex-Dream-Skin) 双布局主题包。

| 月薪喵 · 今日营业（默认横幅） | 月薪喵（沉浸布局） |
|---|---|
| <img src="./presets/preset-yuexinmiao-payday/background.jpg" alt="月薪喵今日营业默认横幅样式预览"> | <img src="./presets/preset-yuexinmiao/background.jpg" alt="月薪喵沉浸布局预览"> |
...
设置完成后，两套样式都会出现在 Dream Skin 的“已保存的主题”中。默认使用“月薪喵 · 今日营业”，用户可随时切换到“月薪喵”，切换不需要重新下载素材或重复设置。
...
- 目标平台：macOS、Windows
- 样式 ID：`preset-yuexinmiao-payday`（默认横幅）、`preset-yuexinmiao`（沉浸布局）
- 主题库：macOS `~/Library/Application Support/CodexDreamSkinStudio/themes/`；Windows `%LOCALAPPDATA%\CodexDreamSkin\themes\`
...
Python 3.9 和 Pillow 11.3.0 只用于维护者重新生成 `background.jpg`，普通用户安装皮肤不会用到。
...
完整安装器会自动处理官方基础运行时，并校验和保存两套预设。macOS 已完整安装基础运行时、但当前任务不适合重启时可使用：

```bash
./scripts/setup-skin-macos.sh --no-apply
```

它会把默认“月薪喵 · 今日营业”设为活动主题，下次从 Codex Dream Skin 启动时直接生效。首次安装仍按上面的安全流程退出 Codex 后重跑 `Setup.command`。
...
仓库只使用 [`source/salary-cat-source.png`](./source/salary-cat-source.png)，不会生成新主体或引入第二张画面。参考截图只决定布局与配色，不会进入主题包。

源图尺寸为 `1942 × 809`，宽高比约 `2.40:1`。两套背景按模板的布局判定分别导出：

- `月薪喵 · 今日营业`（默认）：`2240 × 1600`（`1.40:1`），完整保留源图宽度，等比缩放后在取自原图的浅色画布底部对齐。宽高比低于模板的 `1.45` 宽图扩展阈值，因此首页稳定使用参考图中的横幅卡片，任务页由 `taskMode: banner` 使用顶部横幅。macOS 会自动追加月薪喵样式扩展，用同一张素材图显示深色横幅、顶部标题、右上状态和输入框装饰。
- `月薪喵`：`2560 × 1440`，使用聚焦裁切，舍弃约 26% 横向画幅，主要来自左侧留白，优先放大右侧主体。

两种处理都只做缩放、裁切或同色补边，不改画、不补画，也不会增加真实图像细节。

横幅样式参数：

- 主体焦点：`focusX 0.76` / `focusY 0.86`
- 安全区：`left`
- 任务页模式：`banner`
- 外观：`auto`
...
- `presets/preset-yuexinmiao-payday/`：默认今日营业横幅布局，只含 `background.jpg` 和 `theme.json`
- `presets/preset-yuexinmiao/`：沉浸布局，只含 `background.jpg` 和 `theme.json`
- `assets/salary-cat-extension.css`：macOS 默认样式扩展，显示顶部标题、状态、深色横幅和输入框装饰
- `scripts/build_presets.py`：从唯一素材可重复生成两套 16:9 背景
